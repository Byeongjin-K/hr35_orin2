...
> - **작업 중 호스트가 두 번 재부팅됐다** (09-15 세션 ~18:36, 그리고 오늘 ~13:11).
>   둘 다 내가 유발한 것이 아니고, 매번 `/tmp` 가 비워지며 스크래치 스크립트가 날아갔고
>   붐 드라이버 PID 가 바뀌었다(163882 → 11267). OT 컨테이너는 재시작 정책으로 **ID 를
>   유지한 채** 살아 돌아왔다. **런 산출물은 `/tmp` 가 아니라 `~/data` 에 쓸 것.**
>   현장 실험이 도는 장비에서 원인 미상 재부팅이 두 번이면 그 자체로 볼 만하다.
